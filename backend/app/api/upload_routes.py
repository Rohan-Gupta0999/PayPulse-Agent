...
async def process_transaction_receipts(merchant_id: int, rows: list[dict], field_map: dict, weekly_capital: float, now: datetime):
    ...
    for row in rows:
        ...
        if not phone_norm and not name:
            continue
        ...
        if name:
            this_week_names_norm.add(name.lower())
        ...
        if txn_date > customer_agg[agg_key]["max_date"]:
            customer_agg[agg_key]["max_date"] = txn_date

        if txn_date not in daily_agg:
            daily_agg[txn_date] = {"sales": 0.0, "cost": 0.0, "profit": 0.0, "visits": 0}
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            ...
            # ── BULK OPTIMIZATION: Fetch all existing customers for this merchant in 1 single fast query ──
            existing_res = await session.execute(
                text("SELECT id, name, phone_number FROM customers WHERE merchant_id = :mid"),
                {"mid": merchant_id}
            )
            existing_rows = existing_res.fetchall()
            existing_by_phone = {}
            existing_by_name = {}
            for er in existing_rows:
                p_norm = normalize_phone(er.phone_number or "")
                if p_norm:
                    existing_by_phone[p_norm] = er.id
                if er.name:
                    existing_by_name[er.name.strip().lower()] = er.id

            new_customers_list = []
            for agg_key, cdata in customer_agg.items():
                dt_visit = datetime.combine(cdata["max_date"], datetime.min.time(), tzinfo=timezone.utc)
                matched_id = existing_by_phone.get(cdata["phone_norm"]) or existing_by_name.get(cdata["name"].strip().lower())
                ...
                if matched_id:
                    ...
                    await session.execute(
                        ...
                        {"s": cdata["week_spend"], "v": dt_visit, "cid": matched_id}
                    )
                else:
                    ins_res = await session.execute(
                        text("""
                            INSERT INTO customers (merchant_id, name, phone_number, total_spend, last_visited_at, first_snapshot_id, created_at)
                            VALUES (:m, :n, :p, :s, :v, :snap, NOW()) RETURNING id
                        """),
                        {
                            "m": merchant_id,
                            "n": cdata["name"].strip(),
                            "p": cdata["phone_number"],
                            "s": cdata["week_spend"],
                            "v": dt_visit,
                            "snap": target_week_num,
                        }
                    )
                    new_id = ins_res.scalar_one()
                    cdata["id"] = new_id
                    cdata["is_new"] = True
                    new_customers_list.append({
                        ...
                        "amount_spent": round(cdata["week_spend"], 2),
                        "visit_date": cdata["max_date"].strftime("%d %b %Y")
                    })

            new_count = len(new_customers_list)
            regular_count = max(0, len(customer_agg) - new_count)

            # Clear old pending campaigns
            await session.execute(
                text("DELETE FROM campaigns WHERE merchant_id = :mid AND status = 'PENDING_APPROVAL'"),
                {"mid": merchant_id}
            )

            # Identify at-risk regulars
            absent_query = await session.execute(
                text("""
                    SELECT id, name, phone_number, total_spend, EXTRACT(DAY FROM (:now_ts - last_visited_at))::int AS days_away
                    FROM customers
                    WHERE merchant_id = :m AND last_visited_at <= :c AND total_spend >= :ms
            ...
            for cand in absent_query.fetchall():
                if (normalize_phone(cand.phone_number) in this_week_phones_norm) or ((cand.name or "").strip().lower() in this_week_names_norm):
                    continue
                ...
                ins_camp = await session.execute(
                    text("""
                        INSERT INTO campaigns (merchant_id, customer_id, thread_id, discount_percentage, coupon_code, template_name, status)
                        VALUES (:m, :c, :t, :d, :cp, 'win_back', 'PENDING_APPROVAL') RETURNING id
                    """),
                    {"m": merchant_id, "c": cand.id, "t": tid, "d": disc, "cp": coupon}
                )
                churned.append({
                    "id": cand.id,
                    "name": cand.name,
                    ...
                    "campaign_id": ins_camp.scalar_one(),
                    "thread_id": tid,
                    "discount": disc,
                    "coupon": coupon
                })
    ...
    # Reconcile graph with total unique customers
    ...
    uploaded_graph_data = {
        "weeks": upload_days_labels,
        "sales": upload_days_sales,
        "visits": upload_days_visits,
        "regular": [0]*7,
        "at_risk": [0]*7,
        "profit": upload_days_profit
    }
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            import json
            check_snap = await session.execute(
                text("SELECT id FROM weekly_snapshots WHERE merchant_id = :mid AND week_start_date = :wstart LIMIT 1"),
                {"mid": merchant_id, "wstart": min_date}
            )
            ...
            if existing_snap_row:
                await session.execute(
                    text("""
                        ...
                        SET week_label = :wl, total_sales = :s, weekly_capital = :c, profit = :p,
                            total_customers = :tc, regular_customers = :rc, at_risk_customers = :ar,
                            daily_metrics = :dm, uploaded_at = NOW()
                        WHERE id = :id
                    """),
                    {
                        "id": existing_snap_row.id,
                        "wl": target_week_label,
                        "s": round(total_sales, 2),
                        "c": round(used_capital, 2),
                        "p": calculated_profit,
                        "tc": len(customer_agg),
                        "rc": regular_count,
                        "ar": len(churned),
                        "dm": json.dumps(uploaded_graph_data),
                    }
                )
            else:
                await session.execute(
                    text("""
                        INSERT INTO weekly_snapshots (merchant_id, week_start_date, week_label, total_sales, weekly_capital, profit, total_customers, regular_customers, at_risk_customers, daily_metrics, uploaded_at)
                        VALUES (:m, :ws, :wl, :s, :c, :p, :tc, :rc, :ar, :dm, NOW())
                    """),
                    {
                        "m": merchant_id,
                        "ws": min_date,
            ...
            all_snaps_res = await session.execute(
                text("SELECT * FROM weekly_snapshots WHERE merchant_id = :mid ORDER BY week_start_date ASC, id ASC"),
                {"mid": merchant_id}
            )
            for idx, r in enumerate(all_snaps_res.fetchall()):
                ...
                all_weeks.append({
                    ...
                    "week_num": w_num,
                    "label": re.sub(r"^Week\s*\d+", f"Week {w_num}", r.week_label, flags=re.IGNORECASE),
                    "short_label": f"W{w_num}",
                    ...
                    "kpis": {
                        ...
                        "new_customers": max(0, int(r.total_customers) - int(r.regular_customers)),
    ...
    return {
        ...
        "summary": {
            ...
            "repeat_visits": max(0, len(rows) - len(customer_agg)),
        ...
        "kpis": {
            ...
            "weekly_capital": round(used_capital, 2),
            "regular_customers": regular_count,
            "new_customers": new_count,
            "total_customers": len(customer_agg),
            "at_risk_customers": len(churned),
        },
        "current_week_id": f"week_{len(all_weeks)}",
        "churned_customers": churned,
        "weekly_graph_data": uploaded_graph_data,
        "weeks_breakdown": all_weeks,
    }
